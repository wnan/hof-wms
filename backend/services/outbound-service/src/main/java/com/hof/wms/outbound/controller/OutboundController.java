package com.hof.wms.outbound.controller;
...
import com.hof.wms.outbound.service.OutboundOrderService;
...
public class OutboundController {
    ...
    public ApiResult<PageResult<Map<String, Object>>> list(
        ...
        Page<OutboundOrder> page = orderService.getPage(pageNum, pageSize, keyword, status);
    ...
    public ApiResult<Map<String, Object>> detail(@PathVariable Long id) {
        OutboundOrder order = orderService.getById(id);
        if (order == null) {
            return ApiResult.fail("出库单不存在");
        }
    ...
    public ApiResult<Map<String, Object>> save(@RequestBody Map<String, Object> command) {
        OutboundOrder order = toOrder(command);
        OutboundOrder saved;
        if (order.getId() != null) {
    ...
    public ApiResult<Void> confirm(@PathVariable Long id) {
        ...
        return ApiResult.success("已确认出库", null);
    }

    private Map<String, Object> toMap(OutboundOrder order) {
    ...
    private Map<String, Object> toItemMap(OutboundOrderItem item) {
        Map<String, Object> map = new java.util.HashMap<>();
    ...
    private OutboundOrder toOrder(Map<String, Object> map) {
        OutboundOrder order = new OutboundOrder();
        if (map.get("id") != null) {
            order.setId(((Number) map.get("id")).longValue());
        }
        order.setOrderNo((String) map.get("orderNo"));
        order.setCustomerName((String) map.get("customerName"));
    ...
    private OutboundOrderItem toItem(Map<String, Object> map) {
